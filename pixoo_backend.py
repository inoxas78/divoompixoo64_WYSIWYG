...
# ===========================================================
# ✅ CORE BAKE TASK (FIXED + ATOMIC WRITE)
# ===========================================================
@pyscript_compile
def _worker_bake_task(recipe, sensor_data):
    ...
    if not FONTS:
        FONTS = {"PICO_8": {" ": [0,0,0,0,0,0,0,0,0,3], "?": [1,1,1,0,0,1,0,1,0,0,0,0,0,1,0,3]}}
    ...
    # ---------- output ----------
    os.makedirs(SAVE_DIR_GIF, exist_ok=True)
    out_filename = (recipe.get("output_file", "cuisson_finale.gif") or "cuisson_finale.gif").strip()
    out_path = os.path.join(SAVE_DIR_GIF, out_filename)
    tmp_path = out_path + ".tmp.gif"  # IMPORTANT

    # ---------- background frames ----------
    bg_path_raw = (recipe.get("background_gif", "") or "").strip()
    bg_path = bg_path_raw
    if bg_path.startswith("/local/"):
        bg_path = bg_path.replace("/local/", "/config/www/")
    frames = []
    durations = []

    if bg_path and os.path.exists(bg_path):
        bg_img = Image.open(bg_path)
        if bool(getattr(bg_img, "is_animated", False)):
            for fr in ImageSequence.Iterator(bg_img):
                frames.append(fr.convert("RGBA").resize((64, 64), resample=Image.NEAREST))
                d = fr.info.get("duration", bg_img.info.get("duration", 100))
                try:
                    d = int(d)
                except Exception:
                    d = 100
            ...
            frames.append(bg_img.convert("RGBA").resize((64, 64), resample=Image.NEAREST))
    ...
    for anim in anim_list:
        ...
        if bool(getattr(img, "is_animated", False)):
            for fr in ImageSequence.Iterator(img):
                anim_frames.append(fr.convert("RGBA"))
        else:
    ...
    # ---------- static layer ----------
    static_layer_name = (recipe.get("static_layer", "") or "").strip()
    static_img = None
    ...
    # ---------- master timeline ----------
    master_count = max(len(frames), max((a["count"] for a in loaded_anims), default=1), 1)
    ...
    # ===========================================================
    # ✅ FREEZE SENSOR TEXT (instant T) : plus aucune variation possible
    # ===========================================================
    frozen_text = {}
    for s in (recipe.get("sensors", []) or []):
        eid = (s or {}).get("entity_id")
        if not eid:
            continue
        frozen_text[eid] = str(sensor_data.get(eid, "N/A"))
    ...
    for i in range(master_count):
        ...
        for s in (recipe.get("sensors", []) or []):
            eid = (s or {}).get("entity_id")
            val_str = frozen_text.get(eid, "N/A")   # ✅ ICI: on utilise le snapshot figé
            draw_pixel_text(
                draw_img=base,
                text=str(val_str),
                x=int((s or {}).get("x", 0)),
                y=int((s or {}).get("y", 0)),
                color_hex=(s or {}).get("color", "#ffffff"),
                align=(s or {}).get("align", "left"),
                font_name=(s or {}).get("font", "PICO_8"),
            )

        final_frames.append(base)

    # ---------- save (atomic) ----------
    # on convertit en palette avec transparence index 255 (compat Pixoo)
    # ===========================================================
    # ===========================================================
    # 6) Save GIF (atomic) — force global palette + no optimize
    # ===========================================================
    pal_frames = []
    
    # 1) On aplati sur fond noir + on garde les RGB en mémoire
    rgb_frames = []
    ...
    # 2) On crée UNE palette globale depuis la 1ère frame
    base_pal = rgb_frames[0].convert("P", palette=Image.Palette.ADAPTIVE, colors=256)
    
    # 3) On quantifie TOUTES les frames avec cette palette (palette stable)
    try:
        dither_none = Image.Dither.NONE
    except Exception:
        dither_none = 0  # fallback
    
    for rgb in rgb_frames:
        pal = rgb.quantize(palette=base_pal, dither=dither_none)
    ...
    return f"Cuisson OK: {out_path} (frames={len(pal_frames)}, anims={len(loaded_anims)})"
